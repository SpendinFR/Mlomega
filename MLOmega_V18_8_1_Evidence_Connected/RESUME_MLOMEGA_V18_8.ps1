<# Resume only unresolved V18.8 checkpoints after timeout, PC shutdown or process crash.
...
function Fail([string]$Message) { throw "RESUME V18.8 BLOQUÉ: $Message" }
...
function Assert-ValidatedInstall {
  ...
  if(!$installed){Fail "Aucun rapport d'installation V18.8 réussie. Relance INSTALL avant RESUME."}
...
try {
  ...
  Import-ProjectEnv
  if (!(Test-Path $Python)) { Fail ".venv absent : relance INSTALL_MLOMEGA_V18_8_WINDOWS.ps1" }
  ...
  Ensure-Ollama
  Invoke-Checked { & $Python -m mlomega_audio_elite.cli doctor-core-v18-8 --fail | Out-Host } "Doctor core V18.8 non conforme"
