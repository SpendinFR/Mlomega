<# Start the verified V18.8 core flow: Qdrant, Ollama, Phone Bridge, BrainLive. #>
[CmdletBinding()]
param(
  [string]$PersonId = "me",
  [string]$Title = "BrainLive daily capture",
  [switch]$Restart # Compatibility flag: V18.8 refuses unsafe mid-session replacement.
)
...
$LauncherManifest=Join-Path $RuntimeDir "launcher-v18_8.json"
$InstallSuccess=Join-Path $ProjectRoot ".mlomega_audio_elite\install\installation-success-v18_8.json"
$ReleaseManifest=Join-Path $ProjectRoot "release-manifest-v18_8.json"
function Fail($m){throw "RUN V18.8 BLOQUÉ: $m"}
...
function Export-CoreEnv { if(-not(Test-Path $EnvPath)){Fail "Lance d'abord INSTALL_MLOMEGA_V18_8_WINDOWS.ps1"}; Get-Content $EnvPath | ForEach-Object {if($_ -match '^\s*#' -or $_ -notmatch '='){return};$x=$_ -split '=',2;if($x[0].Trim()){Set-Item "Env:$($x[0].Trim())" $x[1]}}; $env:MLOMEGA_PROJECT_ROOT=$ProjectRoot }
...
function Assert-ReleaseManifestIntegrity {
  if(!(Test-Path $ReleaseManifest)){Fail "Manifest de release V18.8 absent."}
  ...
  if([string]$manifest.version -ne "18.8.1"){Fail "Manifest de release incompatible: $($manifest.version)"}
...
function Assert-ProductionInstall {
  ...
  if(!$installed){Fail "Aucun rapport d'installation V18.8 réussi. Lance INSTALL_MLOMEGA_V18_8_WINDOWS.ps1."}
  if([string]$installed.version -ne "18.8.1"){Fail "Rapport d'installation incompatible: $($installed.version)"}
  ...
  if($envHash -ne ([string]$installed.env_sha256).ToLowerInvariant()){
    Fail ".env a changé depuis la validation. Relance INSTALL_MLOMEGA_V18_8_WINDOWS.ps1 pour revalider la configuration avant capture."
  }
  ...
  if($releaseHash -ne ([string]$installed.release_manifest_sha256).ToLowerInvariant()){
    Fail "Le manifest de release a changé depuis l'installation. Réinstalle/revalide V18.8 avant capture."
  }
...
function Get-RecoveryStatus {
  ...
  if ($LASTEXITCODE -ne 0) { Fail "Impossible de lire l'état de reprise V18.8: $($raw -join ' ')" }
  try { return (($raw -join "`n") | ConvertFrom-Json) }
  catch { Fail "État de reprise V18.8 invalide: $($raw -join ' ')" }
}
...
function Ensure-Ollama {
  ...
  if(-not $bin){Fail "Ollama est absent; relance l'installateur V18.8."}
...
function Start-Bridge {
  ...
  if(!(Test-Path $runner)){Fail "Phone Bridge V18.8 absent."}
...
try {
  ...
  if(!(Test-Path $VenvPython)){Fail ".venv V18.8 absent"}
  New-Item -ItemType Directory -Force -Path $RuntimeDir|Out-Null
  Invoke-Checked { docker compose -f (Join-Path $ProjectRoot "docker-compose.core-v18_8.yml") up -d } "Qdrant ne démarre pas"
  Wait-Http "$($env:MLOMEGA_QDRANT_URL)/collections" 60 "Qdrant"|Out-Null
  Ensure-Ollama
  Invoke-Checked { & $VenvPython -m mlomega_audio_elite.cli doctor-core-v18-8 --fail | Out-Host } "Doctor core V18.8 non conforme"
  ...
  if($recovery.status -eq "resume_required"){
    ...
    Fail "$count reprise(s) durable(s) sont en attente. Pour ne pas démarrer une capture sur des sources non clôturées, exécute d'abord .\RESUME_MLOMEGA_V18_8.ps1 -PersonId $PersonId."
  }
  ...
  if($serviceState.status -eq "running"){
    $hint = if($Restart){" -Restart ne coupe jamais une session active en V18.8."}else{""}
    Fail "Une session BrainLive est déjà active: $($serviceState.service_run_id). Lance STOP_MLOMEGA_V18_8.ps1 puis, si nécessaire, RESUME_MLOMEGA_V18_8.ps1.$hint"
  }
  ...
  $bridge=Start-Bridge
  Invoke-Checked { & $VenvPython -m mlomega_audio_elite.cli doctor-core-v18-8 --fail --check-bridge | Out-Host } "Doctor Phone Bridge V18.8 non conforme"
  ...
  [ordered]@{version="18.8.1";started_at=(Get-Date).ToString("o");person_id=$PersonId;package_date=$packageDate;brainlive_pid=$svc.Id;bridge_pid=if($bridge){$bridge.Id}else{$null};service_run_id=$service.service_run_id;live_session_id=$service.live_session_id;last_heartbeat_at=$heartbeat.last_heartbeat_at;bridge_url=$env:MLOMEGA_PHONE_BRIDGE_URL}|ConvertTo-Json|Set-Content $LauncherManifest -Encoding UTF8
  Write-Host "`nRUN V18.8 ACTIF" -ForegroundColor Green
