<#
MLOmega V18.7 core Windows installer.
...
$TemplatePath = Join-Path $ProjectRoot ".env.core-v18_7.template"
$LockPath = Join-Path $ProjectRoot "requirements-v18_7-windows.lock.txt"
$ComposePath = Join-Path $ProjectRoot "docker-compose.core-v18_7.yml"
$ReleaseManifestPath = Join-Path $ProjectRoot "release-manifest-v18_7.json"
$InstallStatePath = Join-Path $StateDir "install-state-v18_7.json"
$ResumeSecretPath = Join-Path $StateDir "resume-hf-token-v18_7.secure"
$ResumeTaskName = "MLOmega V18.7 Installation Resume"
...
function Fail([string]$Message) { throw "INSTALLATION V18.7 BLOQUÉE: $Message" }
...
function Write-InstallState([string]$State, [string]$Detail = "") {
  try {
    ...
    [ordered]@{version="18.7.1"; state=$State; detail=$Detail; at=(Get-Date).ToString("o"); project_root=$ProjectRoot; person_id=$PersonId; resume_after_reboot=[bool]$ResumeAfterReboot} |
...
function Test-ReleaseManifest {
  if (-not (Test-Path $ReleaseManifestPath)) { Fail "Manifest de release V18.7 absent: $ReleaseManifestPath" }
  ...
  if ([string]$manifest.version -ne "18.7.1") { Fail "Manifest de release incompatible: version '$($manifest.version)'" }
...
function Write-AndroidConfig {
  ...
  $out = Join-Path $ProjectRoot "MLOmega_Phone_Bridge_V18_7\android\mlomega_android_config.env.v18_7.generated"
  @(
    '# Generated by INSTALL_MLOMEGA_V18_7_WINDOWS.ps1. Copy to ~/mlomega_android_config.env in Termux.',
...
function Ensure-OllamaMinimumVersion([object]$OllamaCommand) {
  # qwen3-vl:8b is a required V18.7 model and Ollama's own model page requires
...
function Ensure-BridgeFirewallRule {
  ...
  $name = "MLOmega V18.7 Phone Bridge TCP 8766"
  try {
...
function Assert-BridgePortReady {
  ...
  try {
    ...
    if ($health.ok -and $sameRoot -and [bool]$health.allow_post_stop) {
      Write-Host "Phone Bridge V18.7 déjà actif sur le port $port : réutilisation contrôlée pour le smoke test." -ForegroundColor Yellow
      return
    }
  } catch {}
  $owners = @($listeners | Select-Object -ExpandProperty OwningProcess -Unique) -join ', '
  Fail "Le port Phone Bridge $port est déjà occupé (PID: $owners). Arrête ce processus ou corrige son port avant l'installation; V18.7 refuse de risquer un mauvais receiver."
}
function Assert-PhoneNetworkProfile {
  ...
  if (-not $usable -and $profiles.Count -gt 0) {
    ...
    Fail "Réseau Windows uniquement Public ($names). Le pare-feu V18.7 n'expose pas le Bridge sur Public. Passe le réseau utilisé par le téléphone en Privé ou utilise Tailscale, puis relance."
  }
}

function Assert-NoActiveBrainLive {
  # Never mutate a venv/.env underneath an active capture process. A dead PID
  # after a power loss is harmless; the eventual V18.7 recovery path will own
  # the persisted work safely after installation.
  $manifest = Join-Path $ProjectRoot ".mlomega_audio_elite\runtime\launcher-v18_7.json"
  if (Test-Path $manifest) {
    try {
      ...
      if ($pid -gt 0 -and (Get-Process -Id $pid -ErrorAction SilentlyContinue)) {
        Fail "Une capture BrainLive est active (PID $pid). Lance STOP_MLOMEGA_V18_7.ps1, attends la clôture ou arrête la capture avant une mise à niveau."
      }
    } catch { }
  }
}
function Start-TemporaryBridge([string]$VenvPython) {
  ...
  if (-not (Test-Path $runner)) { Fail "Phone Bridge V18.7 absent du bundle." }
  $port = 8776
  $healthUrl = "http://127.0.0.1:$port/health"
  $smokeRoot = Join-Path $StateDir "bridge-smoke-v18_7"
...
try {
  ...
  if ($freeGB -lt $MinimumFreeGB) { Fail "Espace disque insuffisant sur $($drive.Name): ${freeGB} Go libres; minimum V18.7 = $MinimumFreeGB Go (Python, modèles HF et Ollama)." }
  $ramGB = [math]::Floor((Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory / 1GB)
  if ($ramGB -lt 16) { Fail "RAM insuffisante: ${ramGB} Go détectés; minimum supporté V18.7 = 16 Go." }
  if (-not (Get-Command nvidia-smi -ErrorAction SilentlyContinue)) { Fail "GPU NVIDIA/pilote absent: le profil V18.7 requiert CUDA pour WhisperX/Pyannote." }
  ...
  Write-Step "Configuration V18.7 core (sans Graphiti/Mem0)"
  if (-not (Test-Path $TemplatePath)) { Fail "Template .env V18.7 absent." }
  ...
  $values = @{
    "MLOMEGA_DEPLOYMENT_PROFILE"="CORE_BRAINLIVE_V18_7_PHONE"; "MLOMEGA_GRAPH_BACKEND"="disabled"; "MLOMEGA_MEM0_ENABLED"="false"; "MLOMEGA_STRICT_ELITE"="true";
  ...
  Invoke-Checked { & $NewPython -m pip install -r $LockPath } "Installation des dépendances V18.7 impossible"
  Invoke-Checked { & $NewPython -m pip install --no-deps -e $ProjectRoot } "Installation du paquet local V18.7 impossible"
  ...
  Write-Step "Activation atomique de V18.7"
  ...
  $bridgeSmokeRoot = Join-Path $StateDir "bridge-smoke-v18_7"
  try {
    ...
    $doctorArgs = @("-m","mlomega_audio_elite.cli","doctor-core-v18-7","--fail","--check-bridge","--check-bridge-delivery")
    if (-not $SkipHeavyModelSmoke) { $doctorArgs += @("--check-models","--check-vectors") }
    Invoke-Checked { & $VenvPython @doctorArgs } "Doctor core V18.7 a échoué"
  } finally {
  ...
  $manifest = [ordered]@{version="18.7.1"; installed_at=(Get-Date).ToString("o"); project_root=$ProjectRoot; python=$VenvPython; gpu=$gpu; profile="CORE_BRAINLIVE_V18_7_PHONE"; hf_token_configured=$true; model_smoke_skipped=[bool]$SkipHeavyModelSmoke; android_config=$androidConfig; env_sha256=$envSha256; release_manifest_sha256=$releaseManifestSha256}
  $manifest | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $StateDir "installation-success-v18_7.json") -Encoding UTF8
  Write-InstallState $(if($SkipHeavyModelSmoke){"SYSTEM_READY"}else{"PRODUCTION_READY"}) "doctor core V18.7 terminé"
  if (-not $KeepPreviousVenv -and (Test-Path $VenvOld)) { Remove-Item -Recurse -Force $VenvOld }
  if ($SkipHeavyModelSmoke) {
    Write-Host "`nINSTALLATION TECHNIQUE V18.7 TERMINÉE — les modèles lourds n'ont pas été validés car -SkipHeavyModelSmoke a été demandé." -ForegroundColor Yellow
    Write-Host "Avant une vraie capture, exécute .\DOCTOR_MLOMEGA_V18_7.ps1 -Full -Bridge et corrige toute erreur." -ForegroundColor Yellow
  } else {
    Write-Host "`nINSTALLATION V18.7 RÉUSSIE — lance .\RUN_MLOMEGA_V18_7.ps1 -PersonId $PersonId" -ForegroundColor Green
  }
  if ($androidConfig) { Write-Host "Configuration Android générée: $androidConfig" -ForegroundColor Green }
  else { Write-Host "Bridge PC validé. Pour l'adresse Android, exécute .\EXPORT_PHONE_CONFIG_V18_7.ps1 -Host IP_DU_PC après connexion réseau/Tailscale." -ForegroundColor Yellow }
  ...
  if ($errorText -like "MLOMEGA_REBOOT_REQUIRED:*") {
    try {
      ...
      Write-Host "`nRedémarrage Windows requis. La reprise V18.7 est enregistrée et redémarrera automatiquement à la prochaine ouverture de session." -ForegroundColor Yellow
