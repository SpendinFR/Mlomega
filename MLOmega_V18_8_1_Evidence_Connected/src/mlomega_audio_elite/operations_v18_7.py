...
"""V18.7 operational gates: deterministic doctor, smoke probes and resume.
...
from .runtime_v18_7 import gpu_snapshot, release_gpu_memory
from .utils import now_iso

VERSION = "18.7.1-operations"
...
def _probe_qdrant_read_write() -> _Check:
    ...
    try:
        ...
        _http_json(
            ...
            timeout=max(5.0, cfg.ollama_connect_timeout_s),
            method="PUT",
            payload={"points": [{"id": 1, "vector": [0.0, 1.0], "payload": {"probe": "v18_7"}}]},
        )
        result = _http_json(
        ...
        ok = any(isinstance(point, dict) and str(point.get("id")) == "1" and (point.get("payload") or {}).get("probe") == "v18_7" for point in points)
...
def core_doctor(
    *,
    ...
    """Validate the supported V18.7 core profile without false positives."""
    cfg = get_settings()
    checks: list[_Check] = []
    ...
    path = cfg.root_dir / "runtime" / "doctor_core_v18_7.json"
...
def recovery_status(*, person_id: str) -> dict[str, Any]:
    """Return every unresolved V18.7 boundary for one owner.
    ...
    return {
        ...
        "resume_command": "RESUME_MLOMEGA_V18_7.ps1" if unresolved else None,
    }
